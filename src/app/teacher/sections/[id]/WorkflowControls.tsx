      ...
      {showManageModal && (
        <Modal onClose={() => setShowManageModal(false)} title="Manage Topics">
          <div className="space-y-6">
            ...
            {/* Topics List */}
            <div className="space-y-2">
              <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Available Topics ({topics.length})</p>
              {topics.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {topics.map((t) => (
                    <div key={t} className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg text-sm">
                      <span className="text-slate-700 dark:text-slate-300">{t}</span>
                      <button onClick={() => handleRemoveTopic(t)} className="text-slate-400 hover:text-red-500">
